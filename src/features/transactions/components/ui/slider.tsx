...
export function Slider({ value, onValueChange, min = 0, max = 100, step = 1, className = '' }: SliderProps) {
  return (
    <div className={`relative flex items-center ${className}`} style={{ height: 20 }}>
      <div className="relative w-full h-2 rounded-full overflow-hidden" style={{ background: '#E2E8F0' }}>
        <div className="absolute h-full rounded-full" style={{ background: '#2563EB', width: `${((value[0] - min) / (max - min)) * 100}%` }} />
      </div>
      <input
        type="range"
        ...
        style={{ zIndex: 1 }}
      />
      <div className="absolute rounded-full border-2 border-white shadow-sm" style={{
