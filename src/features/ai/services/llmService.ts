...
export type LLMConversationEntry = { role: 'user' | 'assistant'; content: string }

export async function callLLM(
  userMessage: string,
  history: LLMConversationEntry[] = [],
): Promise<string> {
  const messages = [
    ...history.slice(-8),
  ...
  const directKey = (import.meta as unknown as { env: Record<string, string> }).env.VITE_GROQ_API_KEY

  try {
    let response: Response

    if (directKey) {
      // Dev: call Groq directly (key is in .env.local, never committed)
      ...
      // Production: call through Netlify Function (key stays server-side)
      ...
      return data.content?.trim() ?? fallbackText()
    }
  } catch {
    return fallbackText()
  }
...
export function detectFollowUpSuggestions(content: string): string[] {
  ...
  if (/\bloan\b/.test(lower)) return ['Yes, apply for a loan', 'How much can I borrow?']
  if (/enroll/.test(lower)) return ['Yes, start enrollment', 'Explain plan options']
  if (/withdraw/.test(lower)) return ['Yes, withdraw money', 'What are the penalties?']
  if (/rebalance|portfolio|allocation/.test(lower)) return ['Yes, rebalance now', 'Show current allocation']
  if (/rollover/.test(lower)) return ['Yes, start rollover', 'Learn more about rollovers']
  if (/vest/.test(lower)) return ['Check my vesting', 'What is vesting?']
  return ['What else can you help with?', 'Show main menu']
}
